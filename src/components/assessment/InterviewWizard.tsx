...
const SELECTED: Record<Response, string> = {
  ...
  no: "border-white/30 bg-white/15 text-white",
  na: "border-steel-500 bg-steel-500 text-white",
};
const BASE_OPT = "border-white/15 bg-white/5 text-steel-200/80 hover:border-white/30";
...
export default function InterviewWizard({
  ...
  if (loading) {
    return (
      <div className="mx-auto max-w-xl py-10 text-center">
        ...
        <h2 className="mt-6 text-xl font-bold text-white">Preparing your questions…</h2>
      </div>
    );
  }
  ...
  if (!questions.length) {
    return (
      <div className="mx-auto max-w-xl py-10 text-center">
        <h2 className="text-xl font-bold text-white">Nothing left to confirm</h2>
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <Eyebrow tone="dark">Evidence check</Eyebrow>
      <h1 className="mt-3 text-3xl font-bold text-white sm:text-4xl">Confirm what&rsquo;s documented</h1>
      ...
      <div className="sticky top-2 z-10 mt-5">
        <ScoreGauge
          ...
          className="bg-navy-800/90 shadow-xl shadow-black/40 backdrop-blur"
        />
      </div>

      <div className="mt-5 grid gap-1.5 rounded-xl border border-white/10 bg-white/5 p-4 text-xs text-steel-200/80 sm:grid-cols-2">
        <span>
          <strong className="text-white">Yes, documented</strong> &mdash; you do it, it&rsquo;s
          written down, and you keep records (counts toward audit-ready).
        </span>
        <span>
          <strong className="text-white">Yes, informally</strong> &mdash; you do it, but it&rsquo;s
          not written down / no records.
        </span>
        <span>
          <strong className="text-white">No</strong> &mdash; not in place yet.
        </span>
        <span>
          <strong className="text-white">N/A</strong> &mdash; doesn&rsquo;t apply to you (e.g. no
      ...
      <ul className="mt-6 space-y-3">
        {questions.map((q) => (
          <li key={q.controlId} className="rounded-xl border border-white/10 bg-white/5 p-4 sm:p-5">
            {q.familyName && (
              <p className="text-xs font-semibold uppercase tracking-wide text-steel-200/70">
                {q.familyName}
              </p>
            )}
      ...
      <div className="mt-8 flex items-center justify-between gap-3">
        ...
        <button
          ...
          className="rounded-lg bg-cleared-600 px-6 py-2.5 text-sm font-semibold text-white shadow-sm shadow-cleared-700/20 transition-colors hover:bg-cleared-700 disabled:opacity-50"
        >
